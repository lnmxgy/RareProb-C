...

{
    ...
    string::iterator pos;
   // ifs>>str;
   // if(str.back()=='A' || str.back()=='C') filetype=1;
    ...
    switch(filetype)
    {
        ...
        case 2:
            ifs >> referenceX;
	    referenceX=referenceX.substr(0,referenceX.length()-1);///////////////////////////////////////
            pos = find_if(referenceX.begin(),referenceX.end(),isNot01);
            if(pos != referenceX.end())
            {
                cerr << "genotype file has wrong format on line 1!" << endl;
	...
	for(size_t i = 0;i<(unsigned)numSite;i++)
	{
		...
		/*mafCase[i] = static_cast<double>(vaf[i])/static_cast<double>(numCase);
		if(countControl[i]==0) mafControl[i] =0;
		mafControl[i] =static_cast<double>(0.5);
		maf[i] =  static_cast<double>(mafControl[i]+mafCase[i])/2;*/
